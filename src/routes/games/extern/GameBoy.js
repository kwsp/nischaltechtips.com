...
var GameBoy = function (rom) {
	...
	this.winCtx = this.winCanvas.getContext('2d', { willReadFrequently: true });
	...
	for (var i = 0; i < 40; i++) {
		...
		this.spriteCtx[i] = this.spriteCanvas[i].getContext('2d', { willReadFrequently: true });
	}
	...
	this.LCDStatusAdd = function () {
		...
		if (this.vstatus < 144) {
			...
			if (this.vstatus == 144 && this.hstatus == 0) {
				...
				if (this.fps == 60) {
					if (this.onFPS != null)
						this.onFPS('FPS: ' + Math.round(1000 / (new Date() - this.fpstime)));
					this.fps = 0;
				}
				this.fpstime = new Date();
				this.vblankInt();
			...
			for (var i = 0; sprites[i] != undefined; i++) {
				...
				if ((this.io8bit[0x40] & 4) == 0) {
					this.displaycanvas.drawImage(
						this.spriteCanvas[n],
						0,
						sprites[i].flags & 64 ? 7 - (line - sprites[i].y) : line - sprites[i].y,
						8,
						1,
					...
					this.displaycanvas.drawImage(
						this.spriteCanvas[n],
						0,
						sprites[i].flags & 64 ? 15 - (line - sprites[i].y) : line - sprites[i].y,
						8,
						1,
						sprites[i].flags & 32 ? -sprites[i].x - 8 : sprites[i].x,
						line,
